...
function resolveEquationGrau2() {
    ...
    function resolveVelocidade() {
        const res = document.querySelector('.res6')
        const co = document.getElementById('cmo').value
        const f = document.getElementById('f').value
        if(co.length == 0 || f.length == 0) {
            res.innerHTML = 'Adicione Valores'
            return;
        }
        const velocidade = co * f
        res.innerHTML = `Velocidade de Onda = ${velocidade}`
    }
    function resolveLambda() {
        const vo = document.getElementById('vo').value
        const f = document.getElementById('f2').value
        const res = document.querySelector('.res7')
        if(vo.length == 0 && f.length == 0) {
            res.innerHTML = 'Adicione Valores'
            return;
        }
        const lambda = vo / f
